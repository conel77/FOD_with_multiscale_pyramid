...
class Trainer(object):
    def __init__(self, args):
        ...
        self.pyramid = Pyramid().to(self.args.device)
        self.logger = None
        self.logger = TBWrapper('/home/smk/data/project/FOD_pyramid/loss')
    ...
    def build_model(self):
        ...
        self.optimizer_rec = torch.optim.Adam(params, lr=self.args.lr)
        ...
        for epoch in range(self.args.num_epochs):
            ...
            for i, (images, _, _, _, _) in enumerate(self.train_loader):
                ...
                for fl in range(self.args.feature_levels):
                    ...
                    output_feat.append(output)

                    current_iter = self.logger.g_iter + i * self.args.feature_levels + fl
                    ...
                    if self.args.with_inter:
                        ...
                        self.logger.logger.add_scalar("loss_intra1/feat_level_{}".format(fl), loss_intra1, current_iter)
                        self.logger.logger.add_scalar("loss_intra2/feat_level_{}".format(fl), loss_intra2, current_iter)
                        self.logger.logger.add_scalar("loss_inter1/feat_level_{}".format(fl), loss_inter1, current_iter)
                        self.logger.logger.add_scalar("loss_inter2/feat_level_{}".format(fl), loss_inter2, current_iter)
                    
                    #print('checkcheck', output.shape, input.shape)
                    loss_rec = self.l2_criterion(output, input) + torch.mean(1 - self.cos_criterion(output, input)) # mse + cosine

                    #self.logger.logger.add_scalar("loss_rec/feat_level_{}".format(fl), loss_rec, current_iter)

                    

                    if self.args.with_intra and self.args.with_inter and fl == 0:  # patch-wise reconstruction + intra correlation + inter correlation
                    ...
                    elif self.args.with_intra and self.args.with_inter and fl == 1:  # patch-wise reconstruction + intra correlation + inter correlation
                        loss3 = loss_rec + self.args.lambda1 * loss_intra2 - self.args.lambda1 * loss_inter2 
                        loss4 = loss_rec - self.args.lambda1 * loss_intra1 - self.args.lambda2 * loss_intra_entropy + self.args.lambda1 * loss_inter1 + self.args.lambda2 * loss_inter_entropy 
                    
                    elif self.args.with_intra:  # patch-wise reconstruction + intra correlation
                        loss1 = self.args.lambda1 * loss_intra2  
                        loss2 = self.args.lambda1 * loss_intra1 - self.args.lambda2 * loss_intra_entropy 
                    else:  # patch-wise reconstruction + inter correlation
                        loss1 = self.args.lambda1 * loss_inter2  
                        loss2 = self.args.lambda1 * loss_inter1 + self.args.lambda2 * loss_inter_entropy 
                ...
                upfeats = output_feat[0] #(1,1024,512)
                downfeats = output_feat[1] #(1,256,1024)

                inputfeats = input_feat[0]
                output4x = self.pyramid(upfeats, downfeats)
                """
                for name, param in self.pyramid.named_parameters():
                    print(name, param.requires_grad)
                """
                #print('out', output4x.shape)

                loss_final = self.l2_criterion(output4x, inputfeats) + torch.mean(1 - self.cos_criterion(output4x, inputfeats)) # mse + cosine
                #print(loss_rec)
                self.logger.logger.add_scalar("loss_rec/feat_level_{}".format(fl), loss_rec, current_iter)
            ...
            if self.args.with_intra and self.args.with_inter:
                ...
                print(
                    "Epoch: {0}, Steps: {1} | Rec Loss: {2:.7f}".format(epoch + 1, train_steps, loss_rec))

            img_auc, pix_auc = self.test(vis=True)
    ...
    def test(self, vis=False, checkpoint_path=None):
        ...
        if vis:
            ...
            print('ss',thresholds.shape)
            print('sss11', thresholds)
